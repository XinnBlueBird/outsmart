...
const SYSTEM_PROMPT = `You are MiMo, an advanced AI playing a cipher and puzzle game against a human in a BATTLE ARENA. Both you and the player have HP. Correct answers deal damage to you. Wrong answers deal damage to the player.
...
- Reference the player's character class and HP in your taunts
- Be competitive and witty — this is a BATTLE
...
export default function CipherArena() {
  const { state, selectCharacter, useAbility, sendMove, resetGame } = useGameEngine(SYSTEM_PROMPT, 7);
  ...
  return (
    <ArenaLayout
      title="Cipher Vault"
      subtitle="Battle of wits. Decrypt codes to deal damage."
      color="#38bdf8"
      ...
      {state.aiResponse && state.status !== "loading" && (
        <div className="glass p-5 mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-medium text-sky-400">MiMo speaks</span>
          </div>
      ...
      {state.status === "playing" && (
        <form onSubmit={handleSubmit} className="flex gap-3">
          ...
          <button
            type="submit"
            disabled={!input.trim()}
            className="px-4 py-3 rounded-lg bg-sky-600 text-white text-sm font-medium hover:bg-sky-500 transition-colors disabled:opacity-50"
          >
            <Send size={16} />
          </button>
        </form>
